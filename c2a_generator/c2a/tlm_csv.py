...
def generate(src_dir_path, dest_dir_path, prefix):
    # packet_id 小さい順
...
def generate_(src_file_path, dest_file_path):
    dest_line_len = 18
    dest_line_max = 500
    line_index = 8
...
"""[1:])
        ...
        for row in reader:
            ...
            dest_file.write(f",{row[0]},{row[1]},{row[3]},PACKET,{octet_pos},{bit_pos},{bit_len},{row[4]},{row[5]},{row[6]},{row[7]},{row[8]},\
