...
def generate(src_file_path, dest_file_path):
        ...
        for row in reader:
            ...
            if row[1]:
                ...
                if row[3]:
                    row[3] = 'danger'
                if row[0]:
                    dest_file.write(f",{row[2]},{row[1]},0x{int(row[0]):04X},{int(num_params)},{row[4]},{row[5]},{row[6]},{row[7]},{row[8]},\
{row[9]},{row[10]},{row[11]},{row[12]},{row[13]},{row[14]},{row[15]},{row[3]},restricted,{row[16]},{row[17]}\n")
                else:
                    dest_file.write(f"*,{row[2]},{row[1]},,{int(num_params)},{row[4]},{row[5]},{row[6]},{row[7]},{row[8]},{row[9]},{row[10]},\
